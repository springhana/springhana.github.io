...
description: >
  모던 자바스크립트 Deep Dive: 자바스크립트의 기본 개념과 동작 원리 - 35장 스프레드 문법
---
...
이 문제를 해결하기 위해 배열을 펼쳐서 요소들을 개별적인 값들의 목록으로 만든 후 인수로 전달해야 한다.
...
위 예제의 결과는 기대한 결과가 아니다 세 번째 인수에 배열이 아닌 2, 3 이런 식으로 전달해야 한다. 그렇지 않으면 배열 자체가 추가된다.
...
스프레드 문법을 사용하면 다음과 같이 더욱 간결하고 가독성이 좋게 표현할 수 있다.
...
더 나은 방법으로는 Rest 파라미터를 사용하는 것이다.
...
Rest 프로퍼티와 함께 TC39 프로새스의 stage 4(Finished) 단계에 제안되어 있는 **스프레드 프로퍼티**를 사용하면 객체 리터럴의 프로퍼티 목록에서도 스프레드 문법을 사용할 수 있다. 
